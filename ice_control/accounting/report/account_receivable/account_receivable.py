...
def get_data(filters: frappe._dict) -> tuple[list[dict], dict, dict | None]:
	...
	if not allowed_outlets:
		return [], get_totals([]), get_aging_chart([], filters.end_date)
	...
	sort_field = {
		"customer_code": "gle.party",
		"customer_name": "customer.customer_name",
	}.get(filters.get("sort_by"), "customer.customer_name")
...
def get_totals(data: list[dict]) -> dict:
	...
	return {
		...
		"closing_balance": total_opening + total_debit - total_credit - total_write_off,
	}
...
def get_aging_chart(aging_rows: list[dict], end_date, chart_type=None) -> dict | None:
	...
	for row in aging_rows:
		...
		customer_debits = outstanding_debits.setdefault(customer, [])

		if net_amount > 0:
			advance = unapplied_credits.get(customer, 0)
			applied_advance = min(net_amount, advance)
			net_amount -= applied_advance
			unapplied_credits[customer] = advance - applied_advance

			if net_amount:
				customer_debits.append(
					{
			...
			if credit_to_apply:
				unapplied_credits[customer] = (
					unapplied_credits.get(customer, 0) + credit_to_apply
				)
	...
	for customer_debits in outstanding_debits.values():
		for debit in customer_debits:
