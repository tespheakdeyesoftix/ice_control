        ...
        if doc.get("ref_doctype") == "Sale":
            ...
            if(len(product.product_materials)==0):
                ...
                for pm in product.product_materials:
                    ...
                    stock_location = default_sale_stock_location if default_sale_stock_location else doc.get("stock_location")
                    data = {
                        ...
                        "unit":pm.unit,
                        "quantity": pm.quantity*doc.get("quantity")*doc.get("multiplier"),
...
def update_stocks(doc):
    ...
    if not stock_location_product.get("name",""):
        frappe.get_doc({
...
def get_stock_location_prouct(product,stock_location):
    ...
    if data:
        ...
        purchase_price = frappe.get_cached_value("Product",product,"purchase_price")
        return {"name":"","quantity":0,"cost":purchase_price}
    ...
    if product_code and unit:
        ...
        if data:
            ...
            if frappe.db.exists("Unit",unit):
                return frappe.db.get_value("Unit",unit,"multiplier")
            else:
...
def get_purchase_cost(param: dict[str, Any]) -> Any:
    ...
    if frappe.db.exists("Product",product.get("product_code")):
        base_unit,purchase_price = frappe.db.get_value("Product",product.get("product_code"),["unit","purchase_price"])
        ...
        if vendor_price:
            ...
            if stock_location_product:
                ...
                p_cost = (purchase_price) * multiplier
